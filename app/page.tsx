...
export default function AIChatPage() {
  ...
  // 2. Generate a new ID on every "Access" (page load)
  useEffect(() => {
    const newChatId = `chat-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    setChatId(newChatId);
  }, []);

  // 3. Subscription only starts once chatId is generated
  useEffect(() => {
    if (!chatId) return;
  ...
  // AIChatPage コンポーネント内の他の useState と並べて追加
const [isComposing, setIsComposing] = useState(false);
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
  // 変換中の Enter キー入力を無視する
  if (e.key === "Enter") {
          ...
          {messages.map((m) => {
            ...
            return (
              <div key={m.id} className={`flex w-full ${isAi ? "justify-start" : "justify-end"}`}>
                ...
                >
                  {/* 単なる m.text ではなく ReactMarkdown を使う */}
                  <div className="prose prose-sm max-w-none 
                    [&>ul]:list-disc [&>ul]:ml-4 
                    [&>ol]:list-decimal [&>ol]:ml-4 
                    [&>p]:mb-2 [&>strong]:text-indigo-700"
                >
                  <ReactMarkdown>
                    {m.text}
                  </ReactMarkdown>
